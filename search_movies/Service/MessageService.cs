...
public class MessageService : IHostedService
{
    ...
    public MessageService(IServiceProvider provider)
    {
        ...
        this.httpClient = new HttpClient { BaseAddress = new Uri("http://localhost:5117") };
    }

    // Anslut till RabbitMQ
    public void Connect()
    {
        var factory = new ConnectionFactory { HostName = "10.97.131.29", Port = 5672 };
    ...
    public MovieDto GetMovie(Guid id)
    {
        ...
        try
        {
            var movie = JsonSerializer.Deserialize<MovieDto>(
                json,
